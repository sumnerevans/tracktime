"""
Uses the Chrome Webdriver to get task descriptions from JIRA.
"""
import os
import pickle
import re
...
class JiraSynchroniser(ExternalSynchroniser):
    ...
    def get_task_description(self, entry) -> Optional[str]:
        ...
        cache_file = cache_path.joinpath('jira_selenium_chrome.pickle')
        ...
        if (os.environ.get('JIRA_DISABLE_NETWORK') != '1'
                and not description_cache.get(formatted_task_id)):
            if not self.driver:
                self.init_driver()
